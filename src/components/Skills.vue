    ...
    <div class="holder">
      <form @submit="checkForm">
        ...
        <transition name="alert-in" enter-active-class="animated flipInX" leave-active-class="animated flipOutX">
          <p class="alert" v-if="errors.length"><b>Please correct the following error:</b> {{ errors[0] }}</p>
        </transition>
      </form>
      ...
      <p v-if="skills.length">These are the skills that you possess</p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    checkForm: function (e) {
      ...
      if (!this.skill) {
        this.errors.push('Skill required.');
      }
      else {
        this.addSkill();
      }
